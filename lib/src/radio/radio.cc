...
bool radio::tx(rf_buffer_interface& buffer, const uint32_t& nof_samples_, const srslte_timestamp_t& tx_time_, bool flag)
{
  ...
  // COMMENTING REDIS CODE
    ...
    /* Set a key */
   /* reply = (struct redisReply*)  redisCommand(c,"SET %s %d", "foo", 1);
    printf("SET: %s\n", reply->str);
    freeReplyObject(reply);*/

    reply = (struct redisReply*) redisCommand(c,"GET gain");
    
  // printf("GET: %s\n", reply->str);
   gain_recv=(uint8_t)strtoimax(reply->str,&ptr,0);
 //  printf("\n  current gain %d, recv gain %d",current_tx_gain,gain_recv);
   freeReplyObject(reply);
   if(gain_recv!=current_tx_gain)
   {
     printf("\n radio.cc l315 TX Gain set as %d, previous gain %d",gain_recv, current_tx_gain);
     set_tx_gain(gain_recv);
     
     current_tx_gain=gain_recv;
