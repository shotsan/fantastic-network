...
void gtpu::write_pdu(uint16_t rnti, uint32_t lcid, srslte::unique_byte_buffer_t pdu)
{
  ...
  if (!gtpu_write_header(&header, pdu.get(), gtpu_log)) {
   ...
   char data[SRSLTE_MAX_BUFFER_SIZE_BYTES];
      ...
      testing = reinterpret_cast<char*>(pdu->msg);
      strncpy(data,testing,pdu->N_bytes);
      testing[pdu->N_bytes]='\0';
      printf(" \n total bytes--uplink: %d \n",pdu->N_bytes);
      for(unsigned int i=60;i<pdu->N_bytes;i++)
      {
        printf("%c", testing[i]);
      }*/
...
Santosh: Comment
gtpu::handle_gtpu_s1u_rx_packet is the main interface for packet exchange for both
uplink and downlink
...
//  char data[SRSLTE_MAX_BUFFER_SIZE_BYTES];
  ...
  switch (header.message_type) {
    ...
    case GTPU_MSG_DATA_PDU: {
      ...
      strncpy(data,testing,pdu->N_bytes);
      testing[pdu->N_bytes]='\0';
      printf(" \n total bytes: Downlink: %d \n",pdu->N_bytes);
      for(unsigned int i=52;i<pdu->N_bytes;i++)
      {
        printf("%c", testing[i]);
      }*/
       
    //  printf("\n downlink lcid = %d, rnti =%d\n ",lcid,rnti ); 
     // printf("size %lu, %lu",sizeof(data),sizeof(testing));
      //puts(data);
      }
